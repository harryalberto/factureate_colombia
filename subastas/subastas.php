...
//==== CALCULO LOS FILTROS
$filtros = 'subasta.estado in ('.$v_estados.')';

//==== CALCULO LA CANTIDAD DE REGISTROS CONSIDERANDO FILTROS

$rowcount = $vobj_subasta->get_subastas_v2('COUNT', 0, 0, $filtros,'');
/*--------------------------------------------------------*/
?>
...
?>
    ...
    <div class="frmtransaccion">
        <form name='frm' method='post' id='frm' action="subastas.php">
        <ul>
    ...
    if ($filtroestado == 'on'){
        for ($i=0; $i<count($arrestados); $i++){
            if (in_array($arrestados[$i]['id'], $v_arr_estados))
                echo '
            <li><input type="checkbox" class="formulario_control" name="estados[]" value="'.$arrestados[$i]['id'].'" checked><label style="padding-left:5px;padding-right:5px;">'.$arrestados[$i]['nombre'].'</label></li>';
            else 
                echo '
            <li><input type="checkbox" class="formulario_control" name="estados[]" value="'.$arrestados[$i]['id'].'"><label style="padding-left:5px;padding-right:5px;">'.$arrestados[$i]['nombre'].'</label></li>';
        }
    }
?>
            <li>
    ...
    <div style="overflow:hidden;margin:5px;padding:5px;">
        <div style="overflow:hidden;margin:5px;padding:5px;">
            <table class="tabla_resize">
                <thead>
                    <tr>
                        <th scope="col" class="sort asc">OPERACION ID</th>  <th scope="col" class="sort asc">PAGADOR</th>
                        <th scope="col" class="sort asc">FACTURA</th>       <th scope="col" class="sort asc">ESTADO</th>
                        <th scope="col" class="sort asc">VENCIMIENTO</th>   <th scope="col" class="sort asc">MONTO FINANCIAR</th>
                        <th scope="col" class="sort asc">MONEDA</th>        <th scope="col" class="sort asc">F SUBASTA</th>
    ...
    <script type="text/javascript">
        ...
        function verDetalle(p_subasta_id){
                $('.modal-title').text('DETALLE SUBASTA');
                $('.modal-body').load('subasta_detalle_gestion_modal.php?subastaid='+p_subasta_id,function(){
                    $('#SubastaDetalle').modal({show:true});
                });
        }

        function refresh(){
            location.href = 'subastas.php';
        }
    </script>
</BODY>
</HTML>