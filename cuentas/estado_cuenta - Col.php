...
?>
        <div class="frmtransaccion">
            ...
            <ul style="margin-top:5px;">
                ...
                <li><button type="button" class="btn btn-primary" onclick="cargar_saldo(<?=$arr_saldos[$i]['cuenta_id']?>,<?=$v_medios_pago?>,<?=$v_banco_autom?>)" style="font-size:12px;background-color:var(--color-verde);border:none;">
...
<?php
    ...
    <!--============= llamada al modal -->
    <script>
        function cargar_saldo(p_cuenta_id,p_medios_pago,p_bancos){
            if (p_medios_pago > 0 || p_bancos > 0){
                alert('medios de pago y bancos');
            } else{
                $('.modal-title').text('CARGA DE COMPROBANTE DE DEPOSITO');
                $('.modal-body').load('carga_saldo_inversor_modal.php?cuenta_id='+p_cuenta_id,function(){
