...
<?php
    //==== VERIFICO PERMISOS
    $varr_permisos = $vobj_seg_modal->get_permisos($_SESSION['user']['perfilid']);
    $v_encontrado = 'false';

    for ($i = 0; $i < count($varr_permisos); $i++){
        if ($varr_permisos[$i]['codigo'] == 'MODEMP'){
            $v_encontrado = 'true';
            break;
        }
    }

    if ($v_encontrado == 'true') $v_readonly_datos = '';
    else $v_readonly_datos = 'readonly';
    /*if ($_SESSION['user']['perfilid'] == 14 || $_SESSION['user']['perfilid'] == 10 || $_SESSION['user']['perfilid'] == 15) $v_readonly = 'readonly'; else $v_readonly = ''; // CFO, ANAFIN, CEO
    if ($_SESSION['user']['perfilid'] == 12 || $_SESSION['user']['perfilid'] == 13) $v_readonly = 'readonly'; else $v_readonly = '';    //CLO, ANA LEGAL*/
?>
    ...
    <div id="principal" style="padding-left: 10px;height: 95%;">
        <div class="contenedor_formulario">
            ...
            <div class="contenedor_formulario_column">
                <p style="font-size: 12px;font-weight: bold; margin-top: 10px; padding-top: 10px;">INFORMACION DE LA EMPRESA:</p>
            </div>
...
<?php
    if ($_SESSION['user']['perfilid'] == 14 || $_SESSION['user']['perfilid'] == 10 || $_SESSION['user']['perfilid'] == 12 || $_SESSION['user']['perfilid'] == 13 || $_SESSION['user']['perfilid'] == 15){
        //CFO o ANSLISTA FINANCIERO, CLO, ANALISTA LEGAL,  CEO
        echo '
...
<?php
    if ($_SESSION['user']['perfilid'] == 12 || $_SESSION['user']['perfilid'] == 11 || $_SESSION['user']['perfilid'] == 1 || $_SESSION['user']['perfilid'] == 15 || $_SESSION['user']['perfilid'] == 13){
        echo '
            <div class="contenedor_formulario_column">
                <p style="font-size: 12px;font-weight: bold; margin-top: 10px; padding-top: 10px;">
                    ACCIONISTAS: <button type="button" class="btn btn-primary" style="font-size:11px;background-color:var(--color-azulv2);border:none;" onclick="agregarAccionistas()"><i class="fa-solid fa-user-plus"></i> Accionistas</button>
                </p>
            </div>';
...
?>
            ...
            <div class="contenedor_formulario_column">
                <p style="font-size: 12px;font-weight: bold; margin-top: 10px; padding-top: 10px;">INFORMACION DE CONTACTO:</p>
            </div>
...
<?php
    if ($_SESSION['user']['perfilid'] == 12 || $_SESSION['user']['perfilid'] == 13){
        echo '
            <div class="contenedor_formulario_column">
                <p style="font-size: 12px;font-weight: bold; margin-top: 10px; padding-top: 10px;">EVALUACION LEGAL:</p>
            </div>';
...
<?php
    if ($_SESSION['user']['perfilid'] == 12 || $_SESSION['user']['perfilid'] == 11 || $_SESSION['user']['perfilid'] == 15){ // CLO, COO Y CLEVEL
        if ($t_empresa != 'obligpago'){
            echo '
            <div class="contenedor_formulario_column">
                <div class="formulario_grupo_row" style="width: 200px;">
...
?>
            ...
            <!-- RIESGOS DEL OBLIGADO AL PAGO -->
<?php
    $v_encontrado = 'false';

    for ($i = 0; $i < count($varr_permisos); $i++){
        if ($varr_permisos[$i]['codigo'] == 'MNGRISK'){
            $v_encontrado = 'true';
            break;
        }
    }
    ...
    if (($arr_empresa['t_empresaid'] == 48 || $arr_empresa['t_empresaid'] == 50 || $arr_empresa['t_empresaid'] == 51 || $arr_empresa['t_empresaid'] == 52) && ($v_encontrado == 'true')){
        ...
        if ($v_arr_riesgos['count'] == 0){
            ...
            if ($arr_dias_min['valor'] >= $diasvencimiento){    // esta vigente
                ...
                $v_bg = "background-color:#b30a1f;color:#ffffff;";
                $v_dias_xvencer_factu = $v_dias_xvencer_factu;
            }
            ...
            if ($arr_dias_score['valor'] >= $v_dias_transcurridos_score){    // esta vigente
                ...
                $v_bg_score = "background-color:#b30a1f;color:#ffffff;";
                $v_dias_xvencer_score = $v_dias_xvencer_score;
            }
        }
?>
...
<?php
        ...
        if ($v_arr_riesgos['count'] == 0 || $v_arr_riesgos['riesgo_scoreid'] == 0)
            echo '      <option value="0" selected><-- Seleccionar --></option>';
        else
...
<?php
        if ($v_arr_riesgos['count'] == 0 || $v_arr_riesgos['riesgo_factid'] == 0)
            echo '  <input type="hidden" name="nivel_score_riesgoid_old" id="nivel_score_riesgoid_old" value="0">';
        else
            echo '  <input type="hidden" name="nivel_score_riesgoid_old" id="nivel_score_riesgoid_old" value="'.$v_arr_riesgos['riesgo_scoreid'].'">';
?>
                    
                </div>
                <div class="formulario_grupo_row" style="width: 100px;">
                    <label style="color:#fff;">ACCION</label>
                    <button style="font-size:12px;background-color:var(--color-azulv2);border:none;margin-top: 5px;" type="button" class="btn btn-primary" onclick="guardarRiesgoBuro()">
...
?>
            ...
            <div class="contenedor_formulario_column">
                <div class="formulario_grupo_row" style="width: 250px;">
                    <label for="nivel_factureate_riesgoid">NIVEL RIESGO</label>
...
<?php
        if ($v_arr_riesgos['count'] == 0 || $v_arr_riesgos['riesgo_factid'] == 0)
            echo '  <input type="hidden" name="nivel_factureate_riesgoid_old" id="nivel_factureate_riesgoid_old" value="0">';
        else
            echo '  <input type="hidden" name="nivel_factureate_riesgoid_old" id="nivel_factureate_riesgoid_old" value="'.$v_arr_riesgos['riesgo_factid'].'">';
?>
...
<?php
    echo '  <div class="contenedor_formulario_column">';
            ...
            if ($_SESSION['user']['perfilid'] == 12 || $_SESSION['user']['perfilid'] == 13){       //LEGAL
    ...
    <script type="text/javascript">
        ...
        function guardarRiesgoFactureate(){
            ...
            if (archivo.value == ""){
                ...
                if (nivel_factu.value == "0"){
                    alert("Debe ingresar un nivel de riesgo Factureate");
                    procede = "NO";
                } else {
                    if (msg_factu.value == ""){
                        alert("Debe ingresar un texto de justificacion del nivel de riesgo Factureate");
                        procede = "NO";
                    }
                }
                ...
                if (procede == "SI"){
                    ...
                    var fileInforme = archivo.files[0];
                    var v_empresa_id = $('#empresaid').val();
                    var v_ruc = $('#ruc').val();
                    var v_nivel = $('#nivel_factureate_riesgoid').val();
                    var v_nivel_old = $('#nivel_factureate_riesgoid_old').val();
                    var v_feval = $('#f_evaluacion').val();
                    var v_riesgo_id = $('#riesgo_id').val();
                    var v_justi = $('#factureate_riesgo_justi').val();
                    var v_dias = $('#dias_xvencer').val();
                    var v_empresa = $('#nombre_empresa').val();

                    formData.append('informe_factu_file', fileInforme)
                    formData.append('ruc', v_ruc)
                    formData.append('origen', 'factureate')
                    formData.append('nivel_factureate_riesgoid', v_nivel)
                    formData.append('nivel_factureate_riesgoid_old', v_nivel_old)
                    formData.append('f_evaluacion', v_feval)
                    formData.append('riesgo_id', v_riesgo_id)
                    formData.append('factureate_riesgo_justi', v_justi)
                    formData.append('dias_xvencer', v_dias)
                    formData.append('empresaid', v_empresa_id)
                    formData.append('nombre_empresa', v_empresa)
                    ...
                    .then(data => {
                        if (data.respuesta == '0') alert('Ocurrio un error con los informes adjuntados');
                        else {
                            alert('La evaluación de riesgos fue registrada');
                            var content_pdf = document.getElementById("pdf_factu");

                            content_pdf.innerHTML = data.respuesta;

                            if (v_riesgo_id == 0){
                                var obj_riesgo_id = document.getElementById('riesgo_id');
                                obj_riesgo_id.value = data.resultado;
                            }
                        }
                    })
        ...
        function guardarRiesgoBuro(){
            ...
            if (archivo.value == ""){
                ...
                if (buro.value == "0"){ 
                    alert ("Debe seleccionar un Buro de credito");
                    conforme = "NO";
                } else {
                    if (nivel_buro.value == "0"){ 
                        alert("Debe seleccionar un nivel de riesgo del Buro");
                        conforme = "NO";
                    } else {
                        if (msg_buro.value == ""){ 
                            alert("Debe ingresar un texto de justificacion del nivel de riesgo asignado");
                            conforme = "NO";
                        }
                    }
                }
            ...
            if (conforme == "SI"){
                var v_ruc = $('#ruc').val();
                var v_nivel_riesgo = $('#nivel_score_riesgoid').val();
                var v_nivel_riesgo_old = $('#nivel_score_riesgoid_old').val();
                var v_fevaluacion = $('#f_evaluacion_score').val();
                var v_riesgo_id = $('#riesgo_id').val();
                var v_justi = $('#score_riesgo_justi').val();
                var v_emp_riesgo = $('#emp_riesgoid').val();
                var v_dias_xvencer = $('#dias_xvencer_score').val();
                var v_empresaid = $('#empresaid').val();
                var v_nombre_emp = $('#nombre_empresa').val();

                var formData = new FormData(document.getElementById("frm_modal"));

                formData.append('origen', 'score')
                formData.append('ruc', v_ruc)
                formData.append('nivel_score_riesgoid', v_nivel_riesgo)
                formData.append('nivel_score_riesgoid_old', v_nivel_riesgo_old)
                formData.append('f_evaluacion_score', v_fevaluacion)
                formData.append('riesgo_id', v_riesgo_id)
                formData.append('score_riesgo_justi', v_justi)
                formData.append('emp_riesgoid', v_emp_riesgo)
                formData.append('dias_xvencer_score', v_dias_xvencer)
                formData.append('empresaid', v_empresaid)
                formData.append('nombre_empresa', v_nombre_emp)

                var fileInforme = archivo.files[0];
                formData.append('informe_score_file', fileInforme)
                ...
                .then(data => {
                    if (data.respuesta == '0') alert('Ocurrio un error con los informes adjuntados');
                        ...
                        alert('La evaluación de riesgos fue registrada');
                        var content_pdf = document.getElementById("pdf_buro");
