...
$fnula = strtotime($hoy);
$ffnula = date('Y-m-d', $fnula);

$v_femision_eng = date('Y/m/d');

// RECUPERANDO INFORMACION DE LA FACTURA TEMPORAL
$varr_factura_temp = $objfactura->get_factura_temporal($v_ft_id);

$femision = $varr_factura_temp['f_emision'];
$v_femision_esp = date('d-m-Y',strtotime($varr_factura_temp['f_emision']));

$v_fvencimiento_esp = date('d-m-Y',strtotime($varr_factura_temp['f_vencimiento']));
$fvencimiento = $varr_factura_temp['f_vencimiento'];

$nrofactura = $varr_factura_temp['nro_factura'];
$numerocliente = $varr_factura_temp['cliente_doc'];
$cliente = $varr_factura_temp['cliente_nom'];
$pdfpath = '';
$xml_path = $varr_factura_temp['xml_path'];
$monedaid = $varr_factura_temp['moneda_id'];
$subtotal = $varr_factura_temp['subtotal'];
$subtotal_view = number_format($varr_factura_temp['subtotal'], 2, '.', ',');
$valorventa = $varr_factura_temp['valor_venta'];
$anticipos = $varr_factura_temp['anticipos'];
$anticipos_view = number_format($varr_factura_temp['anticipos'], 2, '.',',');
$descuentos = $varr_factura_temp['descuentos'];
$descuentos_view = number_format($varr_factura_temp['descuentos'], 2, '.',',');
$impuestoventa = $varr_factura_temp['itbis'];
$impuestoventa_view = number_format($varr_factura_temp['itbis'], 2, '.',',');
$total = $varr_factura_temp['total'];
$total_label = number_format($varr_factura_temp['total'],2,'.',',');
$otroscargos = 0;
$otroscargos_view = '';
$otrostributos = $varr_factura_temp['otros'];
$otrostributos_view = number_format($varr_factura_temp['otros'], 2, '.',',');
$retenciones = $varr_factura_temp['retenciones'];
$retenciones_view = number_format($varr_factura_temp['retenciones'],2,'.',',');
$total_neto = $total - $retenciones;
$total_neto_view = number_format($total_neto,2,'.',',');

$tfinid = 0; $v_desc_maximo = 0;

// CALCULO DE FINANCIAMIENTO
$v_inicial = number_format($total_neto * $arrparametros['% FINANCIA']['valornum'],2,'.',',');
$v_inicial_math = $total_neto * $arrparametros['% FINANCIA']['valornum'];
$varr_datos_op = array('documento' => $numerocliente);
$clienteid = $objmaestros->valida_emrpesa_id($varr_datos_op);
...
$v_tasa_dcto_math = (($total_neto - ($v_inicial_math + $v_remanente_math)) / $total_neto) * 100;
$v_tasa_dcto = number_format($v_tasa_dcto_math,2,'.',',');
//=============================

$xmlpath = '';  $readonly = ''; $v_simbolo = ''; $v_condesc_maximo = '';
$nombrexml = '[SIN XML]';
...
?>
    ...
    <form name='frm_factura_modal' method='post' id='frm_factura_modal' enctype="multipart/form-data">
        ...
        <input type="hidden" name="tipoaccion" value="xml">
        <input type="hidden" name="facturaid" value="0">
        <input type="hidden" name="porc_dcto" value="<?=$arrparametros['DESCUENTO PROMEDIO']['valornum']?>">
        <input type="hidden" name="tipo_factura" value="58">    <!-- factura electronica -->
        <input type="hidden" name="nombre_xml" value="<?=$varr_factura_temp['xml_name']?>">
        ...
        <div class="contenedor_formulario">
            <div class="contenedor_formulario_column">
                ...
                <div class="formulario_grupo_row" style="width: 100px;">
                    ...
                    <input type="text" name="factura_id" id="factura_id" class="formulario_control" value="0" style="font-size: 14;font-weight: bold;" readonly>
                </div>
                <div class="formulario_grupo_row" style="width: 100px;">
...
?>
            ...
            <div class="contenedor_formulario_column">
                <div class="formulario_grupo_row" style="width: 120px;">
                    ...
                    <input type="text" id="subtotal_view" name="subtotal_view" placeholder="0.00" class="formulario_control" style="text-align: right;" value="<?= $subtotal_view ?>" onchange="valida_monto_factura()">
                ...
                <div class="formulario_grupo_row" style="width: 120px;">
                    ...
                    <input type="text" id="anticipos_view" name="anticipos_view" placeholder="0.00" class="formulario_control" style="text-align: right;" value="<?= $anticipos_view ?>">
                ...
                <div class="formulario_grupo_row" style="width: 120px;">
                    ...
                    <input type="text" id="descuentos_view" name="descuentos_view" placeholder="0.00" class="formulario_control" style="text-align: right;" value="<?= $descuentos_view ?>">
            ...
            <div class="contenedor_formulario_column">
                ...
                <div class="formulario_grupo_row" style="width: 120px;">
                    ...
                    <input type="text" id="otroscargos_view" name="otroscargos_view" placeholder="0.00" class="formulario_control" style="text-align: right;" value="<?= $otroscargos_view ?>">
                ...
                <div class="formulario_grupo_row" style="width: 120px;">
                    ...
                    <input type="text" id="otrostributos_view" name="otrostributos_view" placeholder="0.00" class="formulario_control" style="text-align: right;" value="<?= $otrostributos_view ?>">
                ...
                <div class="formulario_grupo_row" style="width: 120px;">
                    <label for="retenciones_view">RETENCIONES:</label>
                    <input type="text" name="retenciones_view" id="retenciones_view" placeholder="0.00" class="formulario_control" style="text-align:right;" value="<?=$retenciones_view?>">
                    <input type="hidden" id="retenciones" name="retenciones" value="<?= $retenciones ?>">
                </div>

                <div class="formulario_grupo_row" style="width: 120px;">
                    <label for="total_neto_view">TOTAL NETO:</label>
                    <input type="text" name="total_neto_view" id="total_neto_view" placeholder="0.00" class="formulario_control" style="text-align:right;" value="<?=$total_neto_view?>" readonly>
                    <input type="hidden" id="total_neto" name="total_neto" value="<?= $total_neto ?>">
                </div>
            </div>
            ...
            <div class="contenedor_formulario_column">
                <div class="formulario_grupo_row" style="width: 150px;">
                    ...
                    <select name="tipofinanciamiento" id="tipofinanciamiento" class="formulario_control">
<?php
...
?>
                ...
                <div class="formulario_grupo_row" style="width: 50px;">
                    ...
                    <input id="conmaxdescuento" type="checkbox" class="formulario_control" name="conmaxdescuento" value="1" <?php echo $v_condesc_maximo;?> onchange="javascript:checkdescuento('conmaxdescuento','maxdescuento')">
                </div>
                <div class="formulario_grupo_row" style="width: 150px;">
            ...
            <div class="contenedor_formulario_column">
                <div class="formulario_grupo_column" style="width: 500px;">
                    <label for="facturafile">IMG/PDF FACTURA:</label>
                    <input name="facturafile" id="facturafile" type="file" class="formulario_control" style="background-color:#fff;">
                </div>
            </div>

            <div class="contenedor_formulario_column">
                <div class="formulario_grupo_column" style="width: 500px;">
                    <label for="xmlpath">XML de la factura:<b style="color:#b30a1f;">[**]</b></label>
<?php
        echo '      <span style="margin-right:10px;font-size:20;"><a href="'.$xml_path.'" target="_blank"><i class="fa-solid fa-file-pen"></i></a></span>
                    <input type="hidden" name="xml_path" id="xml_path" value="'.$xml_path.'">
                </div>';
?>
...
?>
    ...
    <script>
        ...
        function calculatotales(dato){                
            ...
            var total_neto, total_neto_f, total_neto_l;
            ...
            if (subtotal < 0){
                ...
                total_neto = Number(total - retenciones);
                total_neto_f = Number(total_neto.toFixed(2));
                total_neto_l = total_neto_f.toLocaleString('en-US');

                monto_inicial = Number(total_neto) * porc_financia;
                tasa_dcto = 0.03;
                monto_remanente = Number(total_neto) - Number(monto_inicial) - (Number(monto_inicial) * Number(v_dias) * Number(v_ted)) - (Number(total_neto) * porc_comision);
                ...
                document.getElementById('total_neto_view').value = total_neto_l;
        ...
        function simbolo_moneda(){
            ...
            if (monto > 0){
                ...
                var total = document.getElementById("total_neto").value;
                ...
                .done(function(rpta){
                    if (rpta == -1){
                        alert('El Importe Total Neto de la factura no cubre el minimo requerido.');
                    }
                });
            }
        }
        ...
        function calcula_financiamiento(){
                ...
                if (v_procesa > 0){
                    ...
                    var v_factura_total = $('#total_neto').val();
        ...
        //==== TRATAMIENTO DE RETENCIONES VIEW
        document.getElementById("retenciones_view").addEventListener("input", function (e) {
                const input = document.getElementById("retenciones_view");
        ...
        function valida_monto_factura(){
            ...
            let total = document.getElementById("total_neto").value;
